...
import { CardActionButton } from "../shared/SocialButton";
type NoteCardProps = {
  content: Content;
      ...
      <>
        ...
        {isExpanded &&
          createPortal(
            ...
            >
              <div
                className={`relative w-full max-w-3xl max-h-[85vh] overflow-y-auto rounded-2xl p-6 pb-8 backdrop-blur-2xl border shadow-2xl ${theme === "light"
                    ? "bg-white/90 border-black/10 shadow-black/20"
                    : "bg-black/90 border-white/10 shadow-white/20"
                  }`}
              >
                <button
                  className={`absolute right-4 top-4 rounded-xl p-2 backdrop-blur-sm border transition-all duration-300 ${theme === "light"
                ...
                <div className="space-y-6">
                  ...
                  >
                    <div
                      className={`whitespace-pre-wrap break-all leading-relaxed text-lg overflow-hidden ${theme === "light" ? "text-black/80" : "text-white/80"
                        }`}
                    >
                      {getNoteContent() || ""}
                    </div>
                  ...
                  {content.tags && content.tags.length > 0 && (
                    <div className="flex flex-wrap gap-3">
                      {content.tags.map((tag, index) => (
                        <span
                          key={index}
                          className={`px-4 py-2 text-sm font-medium rounded-full backdrop-blur-sm border ${theme === "light"
  ...
  return (
    <>
      <div className="relative group w-[280px] h-[240px] flex flex-col pb-7 transition-all duration-500">
        ...
        <div
          className={`absolute inset-0 rounded-2xl backdrop-blur-xl border transition-all duration-500 overflow-hidden ${theme === "light"
        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          <div className="p-3">
            <div
              className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium backdrop-blur-sm border transition-all duration-300 ${theme === "light"
          ...
          <div
            className={`p-4 sm:p-6 flex-1 overflow-y-auto transition-all duration-300 ${onUpdate && !isShared
                ? theme === "light"
          ...
          >
            <div
              className={`whitespace-pre-line break-words ${theme === "light" ? "text-black/70" : "text-white/70"
                }`}
            >
            ...
            {content.tags && content.tags.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {content.tags.map((tag, index) => (
                  <span
                    key={index}
                    className={`text-xs px-2 py-1 rounded-full backdrop-blur-sm border transition-all duration-300 ${theme === "light"
                        ? "bg-purple-500/20 border-purple-500/30 text-purple-700"
                        : "bg-purple-400/20 border-purple-400/30 text-purple-300"
                      }`}
                  >
