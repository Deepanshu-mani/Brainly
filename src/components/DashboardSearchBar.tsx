...
export function DashboardSearchBar({ onAiSearch }: DashboardSearchBarProps) {
  ...
  const handleSearch = () => {
    const input = document.querySelector('input[placeholder="what i post all over days"]') as HTMLInputElement;
    if (input?.value.trim()) {
  ...
  return (
    <div className="mb-12">
      <div className="relative max-w-7xl">
        <div className={`flex rounded-2xl overflow-hidden backdrop-blur-xl ${
          theme === 'light' ? 'border border-black/10 bg-white/10' : 'border border-white/10 bg-black/10'
        }`}>
          {/* Type Dropdown */}
          <div className="relative dropdown-container">
            ...
            >
              <span className="text-lg font-medium">
                {searchType === 'all' ? 'All' : 
                 searchType === 'website' ? 'Web' :
                 searchType === 'youtube' ? 'Video' :
                 searchType === 'twitter' ? 'Tweet' :
                 searchType === 'note' ? 'Note' : 'All'}
              </span>
