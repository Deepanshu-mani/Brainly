          ...
          <div className="bg-white/95 backdrop-blur-lg rounded-2xl border border-white/20 shadow-2xl p-6 sm:p-8 w-full max-w-md mx-4 dark:bg-dark-surface/95 dark:border-dark-border">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-gray-800 dark:text-dark-text-muted capitalize">{type}</p>
              </div>
...
export function UpdateContentModal({
  ...
  async function updateContent() {
    ...
    try {
      const title = titleRef.current?.value;
      const link = linkRef.current?.value;
      const tagsValue = tagsRef.current?.value?.split(',').map(tag => tag.trim()).filter(tag => tag.length > 0) || [];

      await axios.put(`${BACKEND_URL}/content/${contentId}`, {
        title,
        link,
        type,
        tags: tagsValue
        ...
        <div className="w-screen h-screen fixed top-0 left-0 bg-black/50 flex items-center justify-center backdrop-blur-sm z-50 p-4 dark:bg-black/70">
          <div className="bg-white/95 backdrop-blur-lg rounded-2xl border border-white/20 shadow-2xl p-6 sm:p-8 w-full max-w-md mx-4 dark:bg-dark-surface/95 dark:border-dark-border">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-700 block dark:text-dark-text">Title</label>
                <Input placeholder="Enter content title" reference={titleRef} defaultValue={initialTitle} />
              </div>

              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-700 block dark:text-dark-text">Link</label>
                <Input placeholder="Paste your link here" reference={linkRef} defaultValue={initialLink} />
              </div>
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-700 block dark:text-dark-text">Tags (comma separated)</label>
                <Input placeholder="e.g. react, typescript, ai" reference={tagsRef} defaultValue={tags.join(', ')} />
              </div>
