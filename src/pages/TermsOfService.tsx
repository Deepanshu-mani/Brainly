  ...
  return (
    <div className={`transition-colors duration-300 ${
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        {/* Content */}
        <div className={`backdrop-blur-sm rounded-2xl border shadow-lg p-6 sm:p-8 space-y-6 sm:space-y-8 ${
          theme === 'light'
          ...
          <section>
            <h2 className={`text-xl sm:text-2xl font-semibold mb-3 sm:mb-4 ${
              theme === 'light' ? 'text-black' : 'text-white'
            }`}>4. Data Storage and Privacy</h2>
            <p className={`leading-relaxed mb-4 text-sm sm:text-base ${
              theme === 'light' ? 'text-black/70' : 'text-white/70'
            }`}>
              <strong className={theme === 'light' ? 'text-black' : 'text-white'}>Database Storage:</strong> User data including accounts, bookmarks, and preferences are stored in a MongoDB database 
              for demonstration purposes. This data is used solely to showcase the application's functionality.
            </p>
            <p className={`leading-relaxed mb-4 text-sm sm:text-base ${
              theme === 'light' ? 'text-black/70' : 'text-white/70'
            }`}>
              <strong className={theme === 'light' ? 'text-black' : 'text-white'}>User Accounts:</strong> The app requires user registration to demonstrate authentication features. 
              Please use test data only and avoid using real personal information.
            </p>
            <p className={`leading-relaxed mb-4 text-sm sm:text-base ${
              theme === 'light' ? 'text-black/70' : 'text-white/70'
            }`}>
              <strong className={theme === 'light' ? 'text-black' : 'text-white'}>Data Retention:</strong> As this is a demo project, data may be periodically cleared or the database may be reset 
              without notice for maintenance or demonstration purposes.
            </p>
            <p className={`leading-relaxed text-sm sm:text-base ${
              theme === 'light' ? 'text-black/70' : 'text-white/70'
            }`}>
              <strong className={theme === 'light' ? 'text-black' : 'text-white'}>Third-Party Content:</strong> The app displays embedded content from YouTube and Twitter/X. 
