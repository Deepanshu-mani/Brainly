    ...
    return (
      <div className={`min-h-screen flex items-center justify-center transition-colors duration-300 ${
        ...
        <div className="text-center">
          <div className={`animate-spin rounded-full h-12 w-12 border-2 mx-auto mb-6 ${
            theme === 'light' 
        ...
        <div className="mb-12">
          <div className={`flex space-x-2 p-2 rounded-2xl w-fit backdrop-blur-sm border ${
            ...
            {[
              ...
              { key: 'website', label: 'Web Pages', count: data.content.filter(c => c.type === 'website').length },
              { key: 'youtube', label: 'Videos', count: data.content.filter(c => c.type === 'youtube').length },
              { key: 'twitter', label: 'Tweets', count: data.content.filter(c => c.type === 'twitter').length },
              { key: 'note', label: 'Notes', count: data.content.filter(c => c.type === 'note').length },
            ].map((tab) => (
              <button
                key={tab.key}
          ...
          {filteredContent.length === 0 ? (
            ...
            >
              {filteredContent.map((item) => (
                <div key={item._id || item.id || Math.random()} className="mb-6">
                  {item.type === "website" ? (
